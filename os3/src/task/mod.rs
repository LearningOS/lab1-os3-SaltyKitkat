...
lazy_static! {
    ...
    pub static ref TASK_MANAGER: TaskManager = {
        ...
        for (i, t) in tasks.iter_mut().enumerate().take(num_app) {
            t.ctx = TaskContext::goto_restore(init_app_cx(i));
            t.set_ready();
        }
        TaskManager {
            num_app,
...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.set_run();
        let next_task_cx_ptr = &task0.ctx as *const TaskContext;
        drop(inner);
    ...
    fn find_next_task(&self) -> Option<usize> {
        ...
        (current + 1..current + self.num_app + 1)
            ...
            .find(|id| inner.tasks[*id].get_status() == TaskStatus::Ready)
    }
